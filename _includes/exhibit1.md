...
Perfect decoding accuracy indicates $$H(I \vert R) = 0$$—complete determination of representational content by relational structure. Our experiments nominally measure $$H(I \vert R,C)$$ where C represents context, but this conditioning reflects practical limitations of our decoder scope rather than fundamental limitations of relational approaches.

In our experiments, C contains several components: the neural network domain (as opposed to other systems), classification tasks (rather than other computational functions), specific dataset domains (MNIST vs. Fashion-MNIST), network architectures (though these contribute minimally), and the constraint that we decode class identity rather than other representational content. The broadest conceivable context would include only fundamental constraints like our physical universe and choice of modalities.

The dataset classification and cross-architecture transfer results demonstrate that key components of C can be inferred from R itself. We achieve near-perfect accuracy (0.998) distinguishing MNIST from Fashion-MNIST networks based solely on relational structure, showing that task domain—a major component of C—is actually encoded in R. Similarly, strong cross-architecture transfer indicates that architectural differences contribute minimally to context.

This suggests that the distinction between I (content to be decoded) and C (context) is somewhat artificial and arises only because our decoder operates in a limited domain. C could be viewed as part of I that we simply choose not to decode in our experimental setup. The results point toward the possibility of approaching true $$H(I \vert R)$$ rather than just $$H(I \vert R,C)$$—a universal decoder trained on relational structures in larger, multi-modal neural networks could potentially eliminate the need for explicit context conditioning, requiring at most a very broad context consisting of our part of the universe and choice of modalities.

This reframing has implications for interpreting our results. Rather than viewing task-specific geometric patterns as limitations that undermine claims about semantic specificity, we can interpret them as evidence that relational structure encodes multiple levels of information simultaneously—both task domain and specific content within that domain. The ability to decode both MNIST vs. Fashion-MNIST distinctions and digit classifications from the same relational structure suggests that R captures a rich hierarchy of representational information.

Both approaches offer distinct advantages. Learned decoders generalize across representational types and architectures. Geometric matching provides validation when the search space is tractable.

Extensions to decode spatial positions (Exhibit 2), color channels in natural images, or correlation patterns in neural recordings are straightforward.
