...
To pinpoint how much of the decoder’s success comes from the global relational geometry of the output layer, we reran the experiment but supplied the decoder with only the first row of X', the cosine-similarity vector of the target neuron with all other neurons in the input layer, while masking out all pairwise similarities that do not involve the target neuron. Accuracy plummets when that contextual structure is removed (for both vanilla backprop and dropout). This shows that a single neuron’s local neighbourhood is insufficient to accurately determine its class identity; the decoder takes into account how the rest of the output population is organised to disambiguate which digit the target neuron represents.
