...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Gram matrix decoding accuracies across 10 validation networks. Standard deviations reflect variance across network instances. Dropout networks achieve perfect 100\% accuracy with zero variance, indicating completely unambiguous relational encoding of class identity.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Frobenius distances between test and reference Gram matrices across all 10! = 3,628,800 possible permutations of the 10 output neurons, evaluated on each validation network. Each gray bar represents one permutation's distance (averaged across validation networks), while red dots indicate the true permutation. Dropout training (right) creates a substantial gap between the correct permutation and all alternatives, while standard backpropagation (left) shows only small margins, explaining the difference in geometric matching accuracy.}
