...
A potential objection is that our decoding methods—whether geometric matching or learned transformers—constitute external decoders, contradicting our claim that conscious representations require no external interpretation. This objection misunderstands the crucial distinction between arbitrary and structurally grounded decoders.

Consider the JPEG decoder: it is entirely conventional and arbitrary. Nothing about image bit strings themselves determines that they should be decoded via JPEG rather than some other scheme. The decoder must be specified externally by designers who establish the encoding standard. If a different convention had been adopted, the same bit string would represent entirely different images.

Our decoders are fundamentally different. We do not impose an arbitrary mapping from networks to class labels. Instead, we \emph{infer} the decoder by examining networks themselves. The geometric matching approach constructs a reference geometry from networks with known class assignments, then finds which permutation of an unseen network best aligns with this reference. The learned decoder is trained on many network instances to discover consistent relational patterns that indicate class identity.

Crucially, both methods converge on decoders that generalize: the geometric matching achieves perfect accuracy across unseen network instances, and the learned decoder transfers across different architectures. This generalization demonstrates that the decoder is not arbitrary but rather discovers structure that is implicitly present in the networks themselves. The networks encode their representational content in a manner that permits recovery through relational analysis.

The key distinction is between decoders that are \emph{externally imposed conventions} versus decoders that are \emph{structurally necessitated by the representation itself}. A JPEG decoder could be replaced with an entirely different standard without changing the bit strings. Our decoders, in contrast, extract structure that is invariant across network instances—they discover rather than stipulate the mapping. The fact that this mapping can be learned from network exemplars and then applied successfully to novel instances shows it reflects genuine structural properties of the representations.

This is precisely what we mean by "unambiguous representation": the representational content is determined by relational structure in a way that allows convergence on a unique interpretation, rather than being open to arbitrary alternative decodings. An external observer must still perform computation to extract this content, but the content itself is structurally determined, not conventionally assigned.
