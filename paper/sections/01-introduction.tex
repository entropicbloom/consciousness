...
\subsection*{The Intentionality Problem}

We can state the core problem more precisely: Any adequate theory of neural correlates of consciousness must satisfy what we call the \textit{intentionality constraint}. It's not enough to identify a pattern of neural activity that correlates with seeing an apple. The theory must explain why that \textit{particular} pattern corresponds to an apple rather than an orange, a sound, or nothing at all. The conscious content is determinate—when you see an apple, you're not simultaneously seeing an orange or experiencing ambiguity about what you see. Yet the physical substrate (neural activity) appears to be just patterns of electrochemical signals, which seem no more intrinsically ``about'' apples than a bit string is intrinsically about images.

This creates a dilemma: Either (1) we accept that neural representations require arbitrary decoding schemes, making conscious content seem unmotivated and leaving the homunculus problem unsolved, or (2) we identify a type of representation that intrinsically specifies its own content. We pursue option (2).

\subsection*{Our Proposal: Unambiguous Relational Representations}
