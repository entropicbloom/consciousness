...
Using the same decoder architecture and random output-neuron permutations, we trained a binary classifier to distinguish whether a network was trained on MNIST or Fashion-MNIST from output layer weights alone. This tests whether dataset identity is encoded in relational geometry.
...
Relational structure is extracted via column-wise cosine similarities: $X' = X_{\text{norm}}^T X_{\text{norm}}$. Due to the larger neuron count (784 vs.\ 10), we use only the learned decoder (same transformer architecture, now performing regression). Performance is evaluated using $R^2$ score. We apply the same local vs.\ global structure ablation as in Experiment 1 to assess whether the full relational geometry is necessary for spatial position decoding.
